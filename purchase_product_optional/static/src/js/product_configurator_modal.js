...
export const OptionalProductsModalPurchase = Dialog.extend(ServicesMixin, VariantMixin, {
    ...
    init: function (parent, params) {
        ...
        var options = _.extend({
            ...
            buttons: [{
                ...
                // the o_purchase_product_optional_edit class is used for tours.
    ...
    */
    willStart: function () {
        ...
        this.get_supplierinfo_id();
        let supplierinfo_id = {}
        this.supplierinfo_id = supplierinfo_id;
        // let arr = await this.get_product_update_price();
        this.get_product_update_price();
        // let arr = this.get_product_update_price();
        let updatedPrice = this.price;
        self.standard_price = this.price;
    ...
    //     var total = 0;
    //     this.$el.find('.js_product.in_cart').each(function () {
    //         var $product = $(this);
    //         var quantity = parseFloat($product.find('input[name="add_qty"]').val().replace(',', '.') || 1);
    //         var price = parseFloat($product.find('input[name="product_price"]').val().replace(',', '.') || 0);
    //         total += quantity * price;
    //     });
    //     this.$el.find('.js_total_price').text(total.toFixed(2));
    // },
