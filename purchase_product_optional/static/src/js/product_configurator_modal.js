...
export const OptionalProductsModalPurchase = Dialog.extend(ServicesMixin, VariantMixin, {
    ...
    */
    ...
    _onChangeQuantity: function (ev) {
        ...
        var priceTotalElement = document.querySelector('.js_price_total').textContent;
        priceTotalElement = parseFloat(priceTotalElement);
        this.trigger('update_quantity');
        if (this.$modal.find('.js_price_total').length) {
            this.$modal.find('.js_product.in_cart').each(function () {
                var quantity = parseFloat($(this).find('input[name="add_qty"]').last().val().replace(',', '.') || 1);
